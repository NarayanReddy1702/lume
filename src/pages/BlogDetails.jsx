...
export default function BlogDetails() {
  ...
  if (!blog) {
    return (
      <main
        className={`min-h-screen px-6 py-32 transition-colors duration-300 ${
          isDark ? "bg-[#07070a] text-white" : "bg-white text-[#111827]"
        }`}
      >
          ...
          <div className="mt-4 flex items-center gap-3">
            <Link
              ...
              className={`text-[12px] font-medium transition-colors ${
                isDark ? "text-[#9d5cff] hover:text-[#c49aff]" : "text-[#7c3aed] hover:text-[#6d28d9]"
              }`}
            >
  ...
  return (
    <main
      className={`min-h-screen transition-colors duration-300 ${
        isDark ? "bg-[#07070a] text-white" : "bg-[#fafafa] text-[#111827]"
      }`}
    >
      <Navbar animateOnMount={false} theme={isDark ? "dark" : "light"} />
      ...
      <article className="mx-auto w-full max-w-[760px] px-6 py-16 sm:px-8 sm:py-20">
        ...
        <div
          className={`mb-10 mt-10 flex items-center justify-between border-b pb-6 ${
        ...
        >
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[11px] font-medium transition-all duration-200 ${
              isDark
        ...
        <h1
          className={`mt-5 text-[clamp(34px,5vw,54px)] font-medium leading-[1.02] tracking-[-0.035em] ${
        ...
        <div
          className={`mt-6 space-y-5 text-[14px] leading-[1.75] ${
        ...
        <div className="mt-8">
          {(blog.sections || []).map((section, index) => (
            <section key={section.title || index} className="mb-8">
              <h2
                className={`text-[23px] font-medium leading-tight tracking-[-0.025em] ${
                  isDark ? "text-white" : "text-[#111827]"
                }`}
              >
                {section.title}
              ...
              <div
                className={`mt-3.5 space-y-4 text-[14px] leading-[1.75] ${
                  isDark ? "text-white/70" : "text-[#374151]"
                }`}
              >
        ...
        {blog.quote && (
          <blockquote
            className={`my-8 rounded-[8px] border-l-4 px-6 py-5 text-[15px] italic leading-[1.6] ${
              isDark
        ...
        {blog.secondImage && (
          <div
            className={`my-8 overflow-hidden rounded-[10px] border ${
              isDark ? "border-white/10" : "border-black/10"
            }`}
          >
            <img
        ...
        {Array.isArray(blog.contentImages) && blog.contentImages.length > 0 && (
          <div className="my-10 space-y-8">
            {blog.contentImages.map((item, idx) => (
              <figure
                key={idx}
                className={`overflow-hidden rounded-[10px] border p-4 sm:p-6 transition-all duration-300 ${
                  isDark
              ...
              >
                ...
                {item.caption && (
                  <figcaption
                    className={`mt-4 text-[15px] font-medium tracking-tight ${
                      isDark ? "text-white/95" : "text-[#111827]"
                    }`}
                  >
                    {item.caption}
                  </figcaption>
                ...
                {item.content && (
                  <p
                    className={`mt-2.5 whitespace-pre-line text-[13px] leading-[1.7] ${
                      isDark ? "text-white/65" : "text-[#4b5563]"
                    }`}
                  >
                    {item.content}
                  </p>
        ...
        <div
          className={`mx-auto my-8 flex h-[90px] max-w-[560px] items-center justify-center rounded-[8px] border text-center text-[10px] ${
            isDark
        ...
        <section className="mt-8">
          <h2
            className={`text-[22px] font-medium tracking-[-0.025em] ${
              isDark ? "text-white" : "text-[#111827]"
            }`}
          >
            Closing
        ...
        <div
          ref={bottomCtaRef}
          className={`mt-14 rounded-[16px] border p-8 sm:p-10 text-center transition-all duration-300 ${
            isDark
        ...
        >
          <span
            className={`inline-block rounded-full px-3.5 py-1 text-[11px] font-medium tracking-wide ${
              isDark
                ? "border border-[#9d5cff]/30 bg-[#28104a] text-[#c49aff]"
                : "border border-[#e9d5ff] bg-[#f3e8ff] text-[#7c3aed]"
            }`}
          ...
          <h3
            className={`mt-4 text-[24px] sm:text-[30px] font-medium tracking-tight ${
              isDark ? "text-white" : "text-[#111827]"
            }`}
          >
          ...
          <p
            className={`mx-auto mt-2.5 max-w-[500px] text-[13px] sm:text-[14px] leading-relaxed ${
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className={`group flex h-11 w-11 items-center justify-center rounded-full shadow-xl transition-all duration-300 hover:scale-110 active:scale-95 ${
            isDark
